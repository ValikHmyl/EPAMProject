...
<fmt:message key="menu.empty" var="emptyCategory" />
<fmt:message key="menu.add" var="add" />
		...
		<div class="content">
			<div class="row container">
				...
				<div class="col-xs-8 col-sm-9">
					<c:if test="${menu.isEmpty() }">
						<div class="alert alert-warning">${emptyCategory }</div>
					</c:if>
					<c:forEach var="item" items="${menu}">
						<c:if test="${item.status }">
							<div class="col-sm-6">
								<div class="menu-item text-center">
									<img alt="menuImage" src="${pageContext.request.contextPath}/img/menu/${item.imageName}" height="230px">
									<p>${item.name }</p>
									<p>(${item.portion })</p>
									<p>$${item.price }</p>
									<div>
									...
									<button class="add btn btn-default" data-id="${item.id }">${add}</button>
								</div>
							</div>
						</c:if>
					</c:forEach>
